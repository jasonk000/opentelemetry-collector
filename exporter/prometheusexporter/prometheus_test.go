...
func TestPrometheusExporter_endToEndSecondRun(t *testing.T) {
	config := &Config{
		Namespace: "test",
		ConstLabels: map[string]string{
			"foo2":  "bar2",
	...
	for delta := 0; delta <= 20; delta += 10 {
		...
		want := []string{
			`# HELP test_this_one_there_where_ Extra ones`,
			`# TYPE test_this_one_there_where_ counter`,
			fmt.Sprintf(`test_this_one_there_where_{arch="x86",code2="one2",foo2="bar2",os="windows"} %v`, 99+delta),
			fmt.Sprintf(`test_this_one_there_where_{arch="x86",code2="one2",foo2="bar2",os="linux"} %v`, 100+delta),
		}
